...
bar,ax = plt.subplots()
colors =['blue','orange','#FE6100','green','#9A03A2']#,'green','brown','cyan']#
labels = ['Benchmark','RS','CRS','SFC','SFC-G']#,'RS','CRS']#
itype = [', M-I',', S-I']
...
for k in range(5):
    ...
    for i in range(len(all_data)):
        ...
        if k == 0 and (i+1)%2 != 0:
            box1 = ax.boxplot([np.array(raw_data[i])[datas[counter][k],:,1]],medianprops=dict(color="red",linewidth=1),manage_ticks=False,widths=[w], positions=[all_data[i][0,datas[counter][k]]+i*1],patch_artist=True,showfliers=False)
