...
for k in range(5):
    ...
    for i in range(len(all_data)):
        ...
        if k == 0 and (i+1)%2 == 0:
            box1 = ax.boxplot([np.array(raw_data[i])[datas[counter][k],:,1]],medianprops=dict(color="red",linewidth=1),manage_ticks=False,widths=[w], positions=[all_data[i][0,datas[counter][k]]+i*2],patch_artist=True,showfliers=False)#,labels=[labels[int(i/2)]])#+ itype[0])
            ...
            box1 = ax.boxplot(np.array(raw_data[i])[datas[counter][k],:,1],medianprops=dict(color="red",linewidth=1),manage_ticks=False,widths=[w],positions=[all_data[i][0,datas[counter][k]]+i*2],patch_artist=True,showfliers=False)#+ itype[0])
            box1['boxes'][0].set_facecolor(colors[counter])
            box1['boxes'][0].set_edgecolor(colors[counter])
            box1['boxes'][0].set_alpha(0.5)
ax.set_ylim(-0.2)
...
plt.ylabel('Avg. search steps',fontdict=dict(weight='bold',size=20))
