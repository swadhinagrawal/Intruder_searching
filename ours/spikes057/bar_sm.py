...
ax.set_title('Moving Intruder',fontdict=dict(size=14))
# print([labels[int(i/2)] for i in range(0,len(files)) if (i+1)%2 != 0])
ax.legend([b["boxes"][0] for b in boxes_0]+[b["boxes"][0] for b in boxes_m]+[b["boxes"][0] for b in boxes_s], [labels[int(i/3)]+itype[0] for i in range(0,len(files)) if (i)%3 == 0]+[labels[int(i/3)]+itype[1] for i in range(0,len(files)) if (i)%3 == 1]+[labels[int(i/3)]+itype[2] for i in range(0,len(files)) if (i)%3 == 2],prop=dict(size=14),frameon=False,bbox_to_anchor=(1, 1))
