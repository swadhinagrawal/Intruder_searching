...
for i in range(len(all_data)):
    ...
    if i != 0 and i%2 == 0:
        counter += 1
    if (i+1)%2 == 0:
        ind = np.sum(np.array(all_data[i][0])<59)
        ax1.plot(all_data[i][0][:ind],all_data[i][1][:ind], color=colors[counter],label=labels[int(i/2)],linewidth = 3)

ax1.set_xticks(range(0,61,10))
ax1.set_yticks(range(0,220,30))
ax1.set_title('Static Intruder',fontdict=dict(size=14))
# ax1.legend(prop=dict(weight='bold',size=20),frameon=False,bbox_to_anchor=(1.6, 1))
...
# line.savefig(path+'/line_s_inset.pdf',format = "pdf",bbox_inches="tight",pad_inches=0.2)
# plt.show()
