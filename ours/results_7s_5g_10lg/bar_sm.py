...
itype = [', M-I',', S-I']
bench = [3,15,22,31,40,49] # 46, 67, 94, 121, 148
...
ax.set_title('Static vs Moving Intruder',fontdict=dict(size=14))
print([labels[int(i/2)] for i in range(0,len(files)) if (i+1)%2 != 0])
ax.legend([b["boxes"][0] for b in boxes_m]+[b["boxes"][0] for b in boxes_s], [labels[int(i/2)]+itype[0] for i in range(0,len(files)) if (i+1)%2 != 0]+[labels[int(i/2)]+itype[1] for i in range(0,len(files)) if (i+1)%2 != 0],prop=dict(size=14),frameon=False,bbox_to_anchor=(1, 1))
