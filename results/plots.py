...
for k in range(3):
    ...
    for i in range(len(all_data)):
        ...
        if k == 0 and (i+1)%2 != 0:
            ax.bar([all_data[i][0,datas[counter][k]]+i*6],[all_data[i][1,datas[counter][k]]],w,yerr=[all_data[i][2,datas[counter][k]]], color=colors[counter],ecolor='black',alpha=0.5,capsize=5,label=files[i])
        elif k == 0 and (i+1)%2 == 0:
            ax.bar([all_data[i][0,datas[counter][k]]+i*6],[all_data[i][1,datas[counter][k]]],w,yerr=[all_data[i][2,datas[counter][k]]], color=colors[counter],ecolor='black',alpha=0.5,capsize=5,label=files[i],hatch = '..')    
        elif k != 0 and (i+1)%2 == 0:
            ax.bar([all_data[i][0,datas[counter][k]]+i*6],[all_data[i][1,datas[counter][k]]],w,yerr=[all_data[i][2,datas[counter][k]]], color=colors[counter],ecolor='black',alpha=0.5,capsize=5,hatch = '..')    
        else:
