...
for k in range(3):
    ...
    for i in range(len(all_data)):
        ...
        if k == 0 and (i+1)%2 != 0:
            ...
            ax.bar([all_data[i][0,datas[counter][k]]+i*0.4],[all_data[i][1,datas[counter][k]]],w, color=colors[counter],ecolor='black',alpha=0.7,label=labels[int(i/2)] + itype[0])
            ax.errorbar([all_data[i][0,datas[counter][k]]+i*0.4],[all_data[i][1,datas[counter][k]]],yerr=y_error,color="black",capsize=5)
        elif k == 0 and (i+1)%2 == 0:
            y_errormin = [all_data[i][1,datas[counter][k]]]
            y_errormax = [all_data[i][2,datas[counter][k]]]
            y_error = [y_errormin, y_errormax]
            ax.bar([all_data[i][0,datas[counter][k]]+i*0.4],[all_data[i][1,datas[counter][k]]],w,color=colors[counter],ecolor='black',alpha=0.7,label=labels[int(i/2)] + itype[1],hatch = '..')    
            ax.errorbar([all_data[i][0,datas[counter][k]]+i*0.4],[all_data[i][1,datas[counter][k]]],yerr=y_error,color="black",capsize=5)
        elif k != 0 and (i+1)%2 == 0:
            y_errormin = [all_data[i][1,datas[counter][k]]]
            y_errormax = [all_data[i][2,datas[counter][k]]]
            y_error = [y_errormin, y_errormax]
            ax.bar([all_data[i][0,datas[counter][k]]+i*0.4],[all_data[i][1,datas[counter][k]]],w, color=colors[counter],ecolor='black',alpha=0.7,hatch = '..')    
            ax.errorbar([all_data[i][0,datas[counter][k]]+i*0.4],[all_data[i][1,datas[counter][k]]],yerr=y_error,color="black",capsize=5)
        else:
            y_errormin = [all_data[i][1,datas[counter][k]]]
            y_errormax = [all_data[i][2,datas[counter][k]]]
            y_error = [y_errormin, y_errormax]
            ax.bar([all_data[i][0,datas[counter][k]]+i*0.4],[all_data[i][1,datas[counter][k]]],w, color=colors[counter],ecolor='black',alpha=0.7)
            ax.errorbar([all_data[i][0,datas[counter][k]]+i*0.4],[all_data[i][1,datas[counter][k]]],yerr=y_error,color="black",capsize=5)
ax.set_ylim(-0.2)
